            ...
            try {
                ...
                if (optUnderlying && optExpiry && optStrike && /^(CE|PE)$/.test(optType)) {
                    ...
                    if (indexMatch?.exchange && indexMatch?.securityId) {
                        indstocksSymbol = `${indexMatch.exchange}_${indexMatch.securityId}`;
...
app.get(
    ...
    async (req, res) => {
        try {
            ...
            const fallbackSymbol = decodeURIComponent(req.params.symbol ?? "").toUpperCase();
            const fallbackTimeframe = req.query.timeframe || "1minute";
            try {
